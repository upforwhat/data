...
var CheckPostV = function(postid, code)
{
    var listener = document.submitform;
    var result = 0;
    if(listener!=null)
	{
	    listener.hexun_Blog_CheckPostV_onLoad = function(success, responseXML, responseText)
	    { 
            if(success)
            {
                result = responseXML.documentElement.getElementsByTagName('result')[0].firstChild.nodeValue;
                if (result == 1)
                    listener.submit();
                else
                {
	    ...
	    hexunAPI.callMethod('hexun.Blog.CheckPostV',{CheckPostV:1,postid:postid,code:code}, listener);
	}
	else
	{
	    result = 0;
